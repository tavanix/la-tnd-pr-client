...
const Budget = () => {
  ...
  names =
    selectedPositions.length > 0
      ? selectedPositions
          .map((elem) => names.filter((e) => e.positionTitle === elem))
          .flat()
      : names
  ...
  // options for bonus type
  let bonusType = [...new Set(employeesInitialState.map((e) => e.hasBonus))]
  bonusType = bonusType.map((item) => {
    return {
      label: item,
      value: item,
    }
  ...
  useEffect(() => {
    ...
    if (selectedBonusType.length > 0) {
      filteredEmployees = selectedBonusType
        .map((elem) => filteredEmployees.filter((e) => e.hasBonus === elem))
        .flat()
    }
        ...
        content={
          <div className='bg-base-100 grid grid-cols-4 gap-2 h-full'>
            ...
            <MultiSelect
              ...
              setSelected={setSelectedBonusType}
              label='Bonus Type'
            />
          </div>
        }
