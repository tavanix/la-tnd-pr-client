  ...
  return (
    <div className='form-control w-full' ref={dropdownRef}>
      ...
      <div className='relative'>
        <div
          className='input input-bordered w-full cursor-pointer flex flex-wrap gap-1 min-h-[2.5rem] items-center'
          onClick={() => setIsOpen(!isOpen)}
        >
          {selected.length === 0 ? (
            <span className='text-gray-400'>Выберите...</span>
          ) : (
            <span className='p-3 badge badge-outline badge-sm bg-gray-100 text-gray-800 h-8'>
              {selected.length} элементов выбрано
            </span>
          )}
        </div>

        {isOpen && (
          <div className=' mt-1 z-10 bg-white border rounded-md shadow-md w-full max-h-72 overflow-auto'>
            {/* Поиск */}
